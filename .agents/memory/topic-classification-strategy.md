...
**How to apply:** Keep similarity explicitly described as cosine similarity rather than probability. Validate every model or taxonomy change against the normalized-text answer key, including macro-recall, smalltalk rejection, the largest-class baseline, all-five-topic coverage, and the full confusion matrix. For historical comparability, measure v2 against the matching messages in the conversation export (including repeated instances); measure extra158 directly on its unique answer-key texts.